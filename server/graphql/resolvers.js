...
export const resolvers = {
    Query:{
        ...
        actividadAvance:async ()=> {
            try {
                const actiAvance = await ActividadAvance.find();
                return actiAvance;    
            } catch (error) {
        ...
        obtenerActividadAvanceID:async (_,{id})=> {
            try {
                const actiAvanceAlumno = await ActividadAvance.find({idAlumno:id});
    ...
    Mutation:{
        ...
        agregarActividadAvance:async(_,{input})=>{
            const {idAlumno, numActividad} = input;
            //revisar si existe la actividad
            const datos = await ActividadAvance.findOne({idAlumno, numActividad})

            if (!datos) {
                
                try {
                    const nuevoAvance = new ActividadAvance(input);
        ...
        actualizarActividadAvance:async(_,{input,nivel},ctx)=>{
            const act = await ActividadAvance.findOne({idAlumno:ctx.id.toString()});
            // console.log("Las actividades son : ",act);
            // console.log("La actividad a cambiar es: ",input);
            // console.log("Con este nuevo nivel: ", nivel);
